...
const socialFields = [
    ...
    { key: 'social_website', label: 'Situs web', icon: IconWorld, placeholder: 'https://situsaya.com' },
] as const;
...
const form = useForm({
    ...
    section_experience_visible: props.settings.section_experience_visible ?? '1',
...
function uploadSeoImage(event: Event, type: 'og_image' | 'twitter_image' | 'favicon' | 'hero_image' | 'logo_light') {
...
function deleteSeoImage(type: 'og_image' | 'twitter_image' | 'favicon' | 'hero_image' | 'logo_light') {
    ...
    <AdminLayout>
        ...
        <form class="max-w-4xl space-y-6" @submit.prevent="save">
            ...
            <div class="rounded-lg border border-border p-5 space-y-4">
                <h3 class="text-sm font-semibold text-foreground">Visibilitas bagian</h3>
                <p class="text-xs text-muted-foreground">Pilih bagian mana yang ditampilkan di portofolio publik Anda. Bagian yang disembunyikan tidak akan muncul di navbar maupun halaman.</p>
            ...
            <!-- Hero -->
            <div class="rounded-lg border border-border p-5 space-y-4">
                <h3 class="text-sm font-semibold text-foreground">Hero / Judul Utama</h3>
                <div>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Foto profil hero</label>
                    <p class="mb-2 text-xs text-muted-foreground">Format disarankan: gambar persegi (mis. 500×500).</p>
                    <div class="flex items-center gap-4">
                        <div v-if="heroImagePreview" class="flex items-center gap-3">
                            ...
                            <button type="button" class="inline-flex items-center gap-1 rounded-md border border-destructive/30 px-2.5 py-1.5 text-xs text-destructive hover:bg-destructive/10 transition-colors" @click="deleteSeoImage('hero_image')">
                                <IconTrash class="h-3.5 w-3.5" :stroke-width="1.5" /> Hapus
                            </button>
                        </div>
                        <label class="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-input bg-background px-3 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                            <IconUpload class="h-4 w-4" :stroke-width="1.5" /> Unggah foto
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-foreground">Bentuk foto</label>
                    ...
                    <div>
                        <div class="mb-1.5 flex items-center justify-between">
                            <label class="block text-sm font-medium text-foreground">Ukuran</label>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Judul utama</label>
                    <input v-model="form.hero_title" type="text" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Halo, saya Seorang Developer Laravel" />
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Subjudul</label>
                    <textarea v-model="form.hero_subtitle" rows="2" class="w-full resize-none rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Membangun pengalaman web..." />
                </div>
            </div>
            ...
            <!-- Social Links -->
            <div class="rounded-lg border border-border p-5 space-y-4">
                <h3 class="text-sm font-semibold text-foreground">Media sosial</h3>
                <p class="text-xs text-muted-foreground">Kosongkan untuk menyembunyikan. Hanya yang memiliki URL yang ditampilkan.</p>
            ...
            <div class="rounded-lg border border-border p-5 space-y-4">
                ...
                <p class="text-xs text-muted-foreground">Atur judul, deskripsi, dan keyword untuk mesin pencari.</p>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Judul SEO</label>
                    <input v-model="form.seo_title" type="text" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Portofolio Saya — Developer Full Stack" />
                    <span class="mt-1 block text-xs text-muted-foreground">{{ form.seo_title.length }}/200 karakter</span>
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Meta deskripsi</label>
                    <textarea v-model="form.seo_description" rows="2" class="w-full resize-none rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Developer web yang berspesialisasi di Laravel dan Vue.js..." />
                    <span class="mt-1 block text-xs text-muted-foreground">{{ form.seo_description.length }}/500 karakter</span>
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Keywords</label>
                    <input v-model="form.seo_keywords" type="text" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="laravel, vue, developer, portofolio" />
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">URL kanonik</label>
                    <input v-model="form.seo_canonical" type="url" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="https://situsaya.com" />
                </div>
            </div>

            <!-- Logo -->
            <div class="rounded-lg border border-border p-5 space-y-4">
                <h3 class="text-sm font-semibold text-foreground">Logo situs</h3>
                <p class="text-xs text-muted-foreground">Logo ini tampil di navbar publik, sidebar admin, dan halaman login. Jika tidak diunggah, logo bawaan akan digunakan.</p>
                <div class="space-y-3 rounded-lg border border-input p-4">
                    <div>
                        <p class="text-sm font-medium text-foreground">Logo</p>
                        <p class="text-xs text-muted-foreground">Tampil di latar gelap. Gunakan logo versi terang/putih.</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <div v-if="logoLightPreview" class="flex items-center gap-2">
                            <img :src="logoLightPreview.startsWith('blob:') ? logoLightPreview : `/storage/${logoLightPreview}`" alt="Logo" class="h-10 rounded bg-white object-contain p-1 border border-border" />
                            <button type="button" class="inline-flex items-center gap-1 rounded-md border border-destructive/30 px-2.5 py-1.5 text-xs text-destructive hover:bg-destructive/10 transition-colors" @click="deleteSeoImage('logo_light')">
                                <IconTrash class="h-3.5 w-3.5" :stroke-width="1.5" /> Hapus
                            </button>
                        </div>
                        <div v-else class="flex items-center gap-2">
                            <img src="/logo-code.svg" alt="Logo bawaan" class="h-10 w-10 rounded" />
                            <span class="text-xs text-muted-foreground">Logo bawaan (ikon kode)</span>
                        </div>
                        <label class="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-input bg-background px-3 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                            <IconUpload class="h-4 w-4" :stroke-width="1.5" /> Unggah logo
                            <input type="file" accept="image/*" class="hidden" @change="uploadSeoImage($event, 'logo_light')" />
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-lg border border-border p-5 space-y-4">
                ...
                <p class="text-xs text-muted-foreground">Unggah favicon kustom (disarankan: 32×32 atau 512×512 PNG).</p>
                <div class="flex items-center gap-4">
                    ...
                    <label class="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-input bg-background px-3 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                        <IconUpload class="h-4 w-4" :stroke-width="1.5" /> Unggah favicon
            ...
            <div class="rounded-lg border border-border p-5 space-y-4">
                ...
                <p class="text-xs text-muted-foreground">Atur tampilan situs Anda saat dibagikan ke media sosial. Jika dibiarkan kosong, nilai SEO akan digunakan.</p>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Judul OG</label>
                    <input v-model="form.og_title" type="text" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Gunakan judul SEO jika kosong" />
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Deskripsi OG</label>
                    <textarea v-model="form.og_description" rows="2" class="w-full resize-none rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Gunakan meta deskripsi jika kosong" />
                </div>
                <div>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Gambar OG</label>
                    <p class="mb-2 text-xs text-muted-foreground">Disarankan: 1200×630px</p>
                    <div class="flex items-start gap-4">
                        ...
                        <label class="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-input bg-background px-3 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                            <IconUpload class="h-4 w-4" :stroke-width="1.5" /> Unggah gambar
            ...
            <div class="rounded-lg border border-border p-5 space-y-4">
                ...
                <p class="text-xs text-muted-foreground">Atur tampilan situs Anda saat dibagikan ke X/Twitter. Jika dibiarkan kosong, nilai Open Graph akan digunakan.</p>
                <div>
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Judul Twitter</label>
                    <input v-model="form.twitter_title" type="text" class="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Gunakan judul OG jika kosong" />
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Deskripsi Twitter</label>
                    <textarea v-model="form.twitter_description" rows="2" class="w-full resize-none rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Gunakan deskripsi OG jika kosong" />
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-foreground">Gambar Twitter</label>
                    <p class="mb-2 text-xs text-muted-foreground">Jika tidak diunggah, gambar Open Graph akan digunakan.</p>
                    <div class="flex items-start gap-4">
