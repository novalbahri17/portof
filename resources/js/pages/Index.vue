...
type Experience = { id: number; type: 'work' | 'education'; title: string; institution: string; location: string | null; start_date: string; end_date: string | null; is_current: boolean; description: string | null };
...
const allSections = [
  ...
  { id: 'portfolio', label: 'Portofolio', icon: IconBriefcase, visKey: 'portfolio' },
  { id: 'experience', label: 'Pengalaman', icon: IconBriefcase, visKey: 'experience' },
  { id: 'certifications', label: 'Sertifikasi', icon: IconCertificate, visKey: 'certifications' },
...
const timelineItems = computed(() => props.experiences.filter(e => e.type === timelineTab.value));

function formatPeriod(e: Experience): string {
  const opts: Intl.DateTimeFormatOptions = { month: 'short', year: 'numeric' };
  const start = new Date(e.start_date).toLocaleDateString('id-ID', opts);
  const end = e.is_current ? 'Sekarang' : (e.end_date ? new Date(e.end_date).toLocaleDateString('id-ID', opts) : '');
...
const hobbyItems = computed(() => {
  ...
  return props.hobbies.split(/\n/).map(s => s.trim()).filter(Boolean).map((line) => {
    const idx = line.indexOf(' — ');
    return idx > 0 ? { title: line.slice(0, idx), body: line.slice(idx + 3) } : { title: line, body: '' };
  });
});
...
function paginationLabel(label: string): string {
  return label.replace('&laquo; Previous', '« Sebelumnya').replace('Next &raquo;', 'Berikutnya »');
}
...
<main class="min-h-screen overflow-x-hidden bg-background pt-20">
  ...
  <section class="relative z-20 mx-auto max-w-5xl px-6 pt-14 pb-10">
    <div class="grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-14">
      ...
      <div class="text-center lg:text-left">
        <span v-if="heroBadge" class="animate-fade-up inline-flex items-center gap-2 rounded-full border border-primary/25 bg-primary/10 px-3.5 py-1.5 text-xs font-medium text-primary">
          <span class="h-1.5 w-1.5 rounded-full bg-primary" /> {{ heroBadge }}
        </span>

        <h1 class="animate-fade-up mt-5 text-4xl font-bold tracking-tight text-foreground sm:text-5xl lg:text-[3.4rem] leading-[1.08]" style="animation-delay: 0.08s">
          <template v-if="heroAccent">{{ heroTitleLead }}<span class="text-primary">{{ heroAccent }}</span></template>
          <template v-else>{{ heroTitle }}</template>
        </h1>

        <p class="animate-fade-up mx-auto mt-5 max-w-lg text-base text-muted-foreground leading-relaxed sm:text-lg lg:mx-0" style="animation-delay: 0.16s">{{ heroSubtitle }}</p>

        <div class="animate-fade-up mt-8 flex items-center justify-center gap-3 lg:justify-start" style="animation-delay: 0.24s">
          <button @click="scrollTo('contact')" class="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/25 transition-colors hover:bg-primary/90">
            <IconMail class="h-4 w-4" :stroke-width="1.5" /> Hubungi Saya
          </button>
          <button @click="scrollTo('portfolio')" class="inline-flex items-center gap-2 rounded-full border border-border/70 bg-card/50 px-6 py-3 text-sm font-semibold text-foreground backdrop-blur-lg transition-colors hover:border-primary/40 hover:text-primary">
            <IconBriefcase class="h-4 w-4" :stroke-width="1.5" /> Lihat Karya
          </button>
        </div>

        <div v-if="socials.length" class="animate-fade-up mt-8 flex items-center justify-center gap-3 lg:justify-start" style="animation-delay: 0.32s">
          <a v-for="s in socials" :key="s.network" :href="s.url" target="_blank" rel="noopener noreferrer" class="rounded-full border border-border/60 p-2 text-muted-foreground transition-colors hover:border-primary/40 hover:text-primary" :title="s.network">
            <component :is="socialIcons[s.network] ?? IconWorld" class="h-4 w-4" :stroke-width="1.5" />
          </a>
        </div>
      </div>

      <!-- Foto -->
      <div class="animate-fade-up relative flex justify-center lg:justify-end" style="animation-delay: 0.2s">
        <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div class="h-72 w-72 rounded-full bg-primary/20 blur-3xl" />
        </div>

        <div class="relative">
          <div v-if="heroImage" class="relative overflow-hidden ring-1 ring-primary/30 shadow-2xl shadow-primary/20" :class="heroImageShapeClass" :style="{ width: `${heroImageSizePx}px`, height: `${heroImageSizePx}px` }">
            <img :src="`/storage/${heroImage}`" alt="Foto profil" class="h-full w-full object-cover" />
          </div>
          <div v-else class="relative flex items-center justify-center overflow-hidden ring-1 ring-primary/30 shadow-2xl shadow-primary/20" :class="heroImageShapeClass" :style="{ width: `${heroImageSizePx}px`, height: `${heroImageSizePx}px` }">
            <div class="flex h-full w-full items-center justify-center bg-gradient-to-br from-primary/25 via-primary/10 to-transparent">
              <IconCode class="text-primary" :style="{ width: `${heroImageSizePx / 3}px`, height: `${heroImageSizePx / 3}px` }" :stroke-width="1.5" />
            </div>
          </div>

          <div class="absolute -bottom-2 left-1/2 flex -translate-x-1/2 items-center gap-1.5 rounded-full border border-border/70 bg-card/90 px-3 py-1.5 text-[11px] font-medium text-foreground backdrop-blur-lg">
            <span class="h-1.5 w-1.5 rounded-full bg-green-500" /> Open to work
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Marquee teknologi -->
    <div class="animate-fade-up mt-14 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_12%,black_88%,transparent)]" style="animation-delay: 0.4s">
      <div class="flex w-max animate-marquee items-center gap-10">
        <div v-for="loop in 2" :key="loop" class="flex items-center gap-10">
          <div v-for="t in marqueeTech" :key="`${loop}-${t.name}`" class="flex shrink-0 items-center gap-2 text-muted-foreground/70">
            <component :is="t.icon" class="h-5 w-5" :stroke-width="1.5" />
    ...
    <div class="mt-10 flex justify-center">
      <button class="animate-bounce-slow text-muted-foreground/60 transition-colors hover:text-primary" aria-label="Gulir ke bawah" @click="scrollTo('about')">
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
      <div class="rounded-xl border border-border/50 bg-white/50 dark:bg-white/5 backdrop-blur-lg p-6 lg:col-span-2">
        <div class="mb-3 inline-flex items-center gap-1.5 text-xs font-medium text-primary">
          <IconUser class="h-3.5 w-3.5" :stroke-width="1.5" /> Tentang Saya
        </div>
        <div v-if="about" class="prose prose-sm dark:prose-invert text-muted-foreground" v-html="about"></div>
        <p v-else class="text-sm text-muted-foreground">Developer yang berspesialisasi di Laravel dan Vue.js.</p>
      </div>
      <div class="rounded-xl border border-border/50 bg-white/50 dark:bg-white/5 backdrop-blur-lg p-6">
        ...
        <ul v-if="hobbyItems.length" class="space-y-2">
          <li v-for="(h, hi) in hobbyItems" :key="hi" class="flex items-start gap-2.5 rounded-lg bg-white/40 px-3 py-2 text-[13px] leading-relaxed dark:bg-white/5">
            <span class="min-w-0"><span class="font-medium text-foreground">{{ h.title }}</span><span v-if="h.body" class="text-muted-foreground"> — {{ h.body }}</span></span>
          </li>
        </ul>
        <div v-else-if="hobbies" class="prose prose-sm dark:prose-invert text-muted-foreground text-[13px]" v-html="hobbies"></div>
        <p v-else class="text-sm text-muted-foreground">Atur dari admin.</p>
      </div>
    </div>
  </section>

  <!-- ===== SIDE PROJECTS ===== -->
  <section v-if="sectionVisibility.projects" id="side-projects" class="relative z-20 mx-auto max-w-5xl px-6 py-12">
    <div class="mb-7">
      <span class="font-script text-2xl text-primary">{{ sectionMeta['side-projects'].script }}</span>
      <h2 class="text-outline mt-1 text-4xl font-bold tracking-tight sm:text-5xl">{{ sectionMeta['side-projects'].outline }}</h2>
    </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2">
            ...
            <div class="flex flex-col p-5">
              <div v-if="p.featured" class="mb-2 inline-flex w-fit items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-[11px] font-medium text-primary">
                <IconRocket class="h-3 w-3" :stroke-width="1.5" /> Unggulan
              </div>
              ...
              <div class="mt-auto flex gap-3 pt-4">
                ...
                <a v-if="p.repo_url" :href="p.repo_url" target="_blank" class="inline-flex items-center gap-1 text-[13px] text-muted-foreground hover:text-foreground" @click.stop><IconBrandGithub class="h-3 w-3" :stroke-width="1.5" /> Kode</a>
              </div>
            </div>
          </div>
        </article>
    ...
    <div v-else class="rounded-xl border border-dashed border-border/50 bg-white/50 dark:bg-white/5 backdrop-blur-lg p-10 text-center text-sm text-muted-foreground">Segera...</div>
  ...
  <!-- ===== PORTFOLIO ===== -->
  <section v-if="sectionVisibility.portfolio" id="portfolio" class="relative z-20 mx-auto max-w-5xl px-6 py-12">
    <div class="mb-7">
      <span class="font-script text-2xl text-primary">{{ sectionMeta.portfolio.script }}</span>
      <h2 class="text-outline mt-1 text-4xl font-bold tracking-tight sm:text-5xl">{{ sectionMeta.portfolio.outline }}</h2>
    </div>
        ...
        <article v-if="i % 3 === 2" class="group cursor-pointer overflow-hidden rounded-xl border border-border/50 bg-white/50 dark:bg-white/5 backdrop-blur-lg transition-colors hover:border-primary/30 sm:col-span-2" role="link" tabindex="0" @click="goToProject(p.slug)" @keydown.enter.prevent="goToProject(p.slug)">
          <div class="grid grid-cols-1 sm:grid-cols-2">
            ...
            <div class="flex flex-col p-5">
              ...
              <div class="mt-auto flex gap-3 pt-4">
                <a v-if="p.url" :href="p.url" target="_blank" class="inline-flex items-center gap-1 text-[13px] text-primary hover:underline" @click.stop><IconExternalLink class="h-3 w-3" :stroke-width="1.5" /> Lihat Proyek</a>
                <a v-if="p.repo_url" :href="p.repo_url" target="_blank" class="inline-flex items-center gap-1 text-[13px] text-muted-foreground hover:text-foreground" @click.stop><IconBrandGithub class="h-3 w-3" :stroke-width="1.5" /> Kode</a>
              </div>
            </div>
          </div>
  ...
  <section v-if="sectionVisibility.experience" id="experience" class="relative z-20 mx-auto max-w-5xl px-6 py-10">
    <div class="mb-6 text-center">
      <div class="mb-2 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-primary">
        <IconSparkles class="h-3.5 w-3.5" :stroke-width="1.5" /> Perjalanan
      </div>
      <h2 class="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Pengalaman &amp; Pendidikan</h2>
      <div class="mx-auto mt-3 h-1 w-16 rounded-full bg-gradient-to-r from-primary to-primary/30" />
    </div>

    <!-- Tab toggle -->
    <div class="mb-8 flex justify-center">
      <div class="inline-flex rounded-xl border border-border/50 bg-white/50 p-1 backdrop-blur-lg dark:bg-white/5">
        <button
          v-for="tab in timelineTabs"
          :key="tab.key"
          type="button"
          class="inline-flex items-center gap-1.5 rounded-lg px-5 py-2 text-sm font-medium transition-all"
          :class="timelineTab === tab.key ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'"
          @click="timelineTab = tab.key"
        >
          <component :is="tab.icon" class="h-4 w-4" :stroke-width="1.5" /> {{ tab.label }}
        </button>
      </div>
    </div>
    ...
    <div v-if="timelineItems.length" class="relative">
      ...
      <div class="absolute top-0 bottom-0 left-4 w-px bg-gradient-to-b from-primary/60 via-border to-transparent md:left-1/2 md:-translate-x-1/2" />
      ...
      <div class="space-y-8">
        ...
        >
          <!-- Node dot -->
          <span class="absolute left-4 top-2 z-10 flex h-4 w-4 -translate-x-1/2 items-center justify-center md:left-1/2">
            <span class="absolute inline-flex h-4 w-4 animate-ping rounded-full bg-primary/40" />
            <span class="relative inline-flex h-3 w-3 rounded-full border-2 border-background bg-primary" />
          </span>
          ...
          <article
            ...
            :class="i % 2 === 0 ? 'md:col-start-1' : 'md:col-start-2'"
          >
            <div class="mb-1 flex flex-wrap items-center gap-2 text-xs font-medium text-primary">
              <span>{{ formatPeriod(e) }}</span>
              <span v-if="e.is_current" class="inline-flex items-center gap-1 rounded-full bg-green-500/10 px-2 py-0.5 text-[11px] text-green-600 dark:text-green-400">
                <span class="h-1.5 w-1.5 rounded-full bg-green-500" /> Aktif
              </span>
            </div>
            <h3 class="text-base font-semibold text-foreground">{{ e.title }}</h3>
            <div class="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-sm text-muted-foreground">
              <span class="font-medium text-primary/90">{{ e.institution }}</span>
              <span v-if="e.location" class="inline-flex items-center gap-1 text-[13px]">
                <IconMapPin class="h-3 w-3" :stroke-width="1.5" /> {{ e.location }}
              </span>
            </div>
            <ul v-if="splitDescription(e.description).length" class="mt-3 space-y-1.5">
              <li v-for="(line, li) in splitDescription(e.description)" :key="li" class="flex gap-2 text-[13px] leading-relaxed text-muted-foreground">
                <span class="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-primary/50" />
                <span>{{ line }}</span>
              </li>
            </ul>
          </article>
    ...
    <div v-else class="rounded-xl border border-dashed border-border/50 bg-white/50 dark:bg-white/5 backdrop-blur-lg p-10 text-center text-sm text-muted-foreground">
      {{ timelineTab === 'work' ? 'Belum ada pengalaman kerja.' : 'Belum ada data pendidikan.' }}
    </div>
  </section>

  <!-- ===== CERTIFICATIONS ===== -->
  <section v-if="sectionVisibility.certifications" id="certifications" class="relative z-20 mx-auto max-w-5xl px-6 py-12">
    <div class="mb-7 flex flex-wrap items-end justify-between gap-4">
      <div>
        <span class="font-script text-2xl text-primary">{{ sectionMeta.certifications.script }}</span>
        <h2 class="text-outline mt-1 text-4xl font-bold tracking-tight sm:text-5xl">{{ sectionMeta.certifications.outline }}</h2>
      </div>
        ...
        <article v-for="certification in certifications" :key="certification.id" class="w-full shrink-0 px-1 sm:w-1/2 lg:w-1/3">
          <div class="group flex h-full flex-col overflow-hidden rounded-xl border border-border/50 bg-white/50 backdrop-blur-lg transition-colors hover:border-primary/30 dark:bg-white/5">
            ...
            <div class="flex h-full flex-col p-5">
              ...
              >
                <IconExternalLink class="h-3.5 w-3.5" :stroke-width="1.5" /> Lihat Sertifikat
              </a>
            </div>
          </div>
        </article>
  ...
  <!-- ===== BLOG ===== -->
  <section v-if="sectionVisibility.blog" id="blog" class="relative z-20 mx-auto max-w-5xl px-6 py-12">
    <div class="mb-7">
      <span class="font-script text-2xl text-primary">{{ sectionMeta.blog.script }}</span>
      <h2 class="text-outline mt-1 text-4xl font-bold tracking-tight sm:text-5xl">{{ sectionMeta.blog.outline }}</h2>
    </div>
    <div v-if="blogs.data.length" class="grid grid-cols-1 gap-4 sm:grid-cols-3">
      <Link v-for="b in blogs.data" :key="b.id" :href="`/blog/${b.slug}`" class="group overflow-hidden rounded-xl border border-border/50 bg-card/50 backdrop-blur-lg transition-colors hover:border-primary/40">
        ...
        <div class="flex flex-col p-5">
          <div class="mb-1.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
            ...
            <span v-if="b.published_at">{{ new Date(b.published_at).toLocaleDateString('id') }}</span>
          </div>
  ...
  <!-- ===== CONTACT ===== -->
  <section v-if="sectionVisibility.contact" id="contact" class="relative z-20 mx-auto max-w-5xl px-6 py-14">
    <div class="mb-10 text-center">
      <span class="font-script text-3xl text-primary">{{ sectionMeta.contact.script }}</span>
      <h2 class="mt-1 text-4xl font-bold tracking-tight text-foreground sm:text-5xl">Mari Bekerja Sama</h2>
      <p class="mx-auto mt-4 max-w-lg text-sm leading-relaxed text-muted-foreground">Tulis pesan untuk saya dan mari diskusikan bagaimana saya bisa membantu membangun sesuatu yang luar biasa.</p>
    </div>
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-5">
      ...
      <form class="rounded-xl border border-border/50 bg-white/50 dark:bg-white/5 backdrop-blur-lg p-6 lg:col-span-3" @submit.prevent="submitContact">
        <div class="grid gap-3 sm:grid-cols-2">
          <div>
            <label for="c-name" class="mb-1 block text-[13px] font-medium text-foreground">Nama</label>
            <input id="c-name" v-model="contactForm.name" type="text" required class="w-full rounded-lg border border-input bg-white/60 dark:bg-white/5 backdrop-blur px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Nama Anda" />
        ...
        <div class="mt-3">
            <label for="c-subject" class="mb-1 block text-[13px] font-medium text-foreground">Subjek</label>
            <input id="c-subject" v-model="contactForm.subject" type="text" class="w-full rounded-lg border border-input bg-white/60 dark:bg-white/5 backdrop-blur px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Ingin membahas apa?" />
        </div>
        <div class="mt-3">
          <label for="c-message" class="mb-1 block text-[13px] font-medium text-foreground">Pesan</label>
          <textarea id="c-message" v-model="contactForm.message" required rows="3" class="w-full resize-none rounded-lg border border-input bg-white/60 dark:bg-white/5 backdrop-blur px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Ceritakan tentang proyek Anda..." />
        ...
        <button type="submit" :disabled="contactForm.processing" class="mt-3 inline-flex items-center gap-2 rounded-lg bg-primary px-5 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50">
          <IconSend class="h-3.5 w-3.5" :stroke-width="1.5" />{{ contactForm.processing ? 'Mengirim...' : 'Kirim pesan' }}
        </button>
      </form>
    </div>
  </section>
  ...
  <footer class="relative z-20 mt-12 border-t border-border/40 py-12">
    <div class="mx-auto grid max-w-5xl grid-cols-1 gap-8 px-6 sm:grid-cols-[1.6fr_1fr_1fr]">
      <div>
        <SiteLogo img-class="h-9 w-9" />
        <p class="mt-4 max-w-xs text-[13px] leading-relaxed text-muted-foreground">{{ heroSubtitle }}</p>
      </div>
      <div>
        <p class="mb-3 text-[11px] font-semibold tracking-widest text-muted-foreground/70 uppercase">Halaman</p>
        <ul class="space-y-2 text-[13px] text-muted-foreground">
          <li v-for="s in visibleFooterSections" :key="s.id">
            <button class="transition-colors hover:text-primary" @click="scrollTo(s.id)">{{ s.label }}</button>
          </li>
        </ul>
      </div>
      <div>
        <p class="mb-3 text-[11px] font-semibold tracking-widest text-muted-foreground/70 uppercase">Sosial</p>
        <ul class="space-y-2 text-[13px] text-muted-foreground capitalize">
          <li v-for="s in socials" :key="s.network">
            <a :href="s.url" target="_blank" rel="noopener noreferrer" class="transition-colors hover:text-primary">{{ s.network }}</a>
          </li>
          <li v-if="!socials.length" class="text-muted-foreground/60">Belum ada tautan</li>
        </ul>
      </div>
    </div>
    <div class="mx-auto mt-10 max-w-5xl border-t border-border/30 px-6 pt-6 text-[12px] text-muted-foreground/70">
      &copy; {{ new Date().getFullYear() }} — Dibuat dengan Laravel + Vue
    </div>
  </footer>

  <!-- Scroll to top -->
  <button class="fixed right-5 bottom-5 z-30 rounded-full border border-border/50 bg-white/50 dark:bg-white/5 backdrop-blur-lg p-2.5 shadow-lg transition-all hover:bg-white/70 dark:hover:bg-white/10" @click="scrollTo('about')" aria-label="Kembali ke atas">
